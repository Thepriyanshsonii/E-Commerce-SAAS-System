...
JWT_SECRET = os.getenv("JWT_SECRET", "supersecret_bharatbasket_key_123")
...
def login():
    ...
    if (email == 'admin' or email == 'admin@bharatbasket.com') and password == 'admin123':
...
def send_otp_route():
    ...
    # Send email
    subject = "BharatBasket Verification Code"
    body = f"""Hello,

Welcome to BharatBasket.
...
def user_login_route():
    ...
    if (name == 'admin' or name == 'admin@bharatbasket.com') and mobile == 'admin123':
        from backend.utils.audit import log_admin_action
        log_admin_action("Admin Login", "Admin Authentication", "Admin login successful")
        payload = {
            "user_id": "admin_user",
...
def forgot_password():
    ...
    # Send email
    subject = "BharatBasket Password Reset Code"
    body = f"""Hello,
    
We received a request to reset the password for your BharatBasket account.
...
def checkout_login_route():
    ...
    if not user_obj:
        ...
        user_email = email if email else f"{phone}@bharatbasket.com"
...
def test_smtp():
    ...
    subject = "BharatBasket SMTP Test Email"
    body = """Hello,
