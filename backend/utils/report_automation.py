...
def seed_default_settings():
    ...
    try:
        ...
        if res == 0:
            ...
            smtp_email = os.environ.get("MAIL_USERNAME") or os.environ.get("EMAIL_ADDRESS") or "bharatbasket2@gmail.com"
...
def write_dashboard_sheet(writer, stats, month_name, year):
    ...
    title_cell = sheet["A1"]
    title_cell.value = f"BHARATBASKET - MONTHLY BUSINESS REPORT ({month_name.upper()} {year})"
...
def generate_report_file(m, y):
    ...
    filename = f"BharatBasket_Monthly_Report_{month_name}_{y}.xlsx"
...
def get_report_settings():
    ...
    if not settings["smtp_email"]:
        settings["smtp_email"] = os.environ.get("MAIL_USERNAME") or os.environ.get("EMAIL_ADDRESS") or "bharatbasket2@gmail.com"
...
def send_report_email(file_path, filename, month_name, year):
    ...
    msg['From'] = smtp_email
    msg['To'] = owner_email
    msg['Subject'] = f"BharatBasket Monthly Business Report - {month_name} {year}"
    
    body = f"""Hello Owner,
 
Please find attached the automated BharatBasket Monthly Business Report for {month_name} {year}.
 
This email and report were generated automatically.
 
Warm regards,
BharatBasket Automation System"""
