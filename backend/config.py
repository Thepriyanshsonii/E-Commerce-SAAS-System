import os
from dotenv import load_dotenv

load_dotenv()

from urllib.parse import urlparse, urlunparse, parse_qs, urlencode
import socket

def resolve_neon_uri(uri):
    if not uri:
        return uri
    try:
        parsed = urlparse(uri)
        if parsed.hostname and (parsed.hostname.endswith('.neon.tech') or 'neon' in parsed.hostname):
            # Force IPv4 lookup — required for Neon DB on some cloud hosts (e.g. Render, Oracle)
            addrinfo = socket.getaddrinfo(parsed.hostname, parsed.port or 5432, socket.AF_INET, socket.SOCK_STREAM)
            if addrinfo:
                ipv4 = addrinfo[0][4][0]
                endpoint_id = parsed.hostname.split('.')[0]
                
                port_str = f":{parsed.port}" if parsed.port else ""
                auth_str = ""
                if parsed.username:
                    auth_str += parsed.username
                    if parsed.password:
                        auth_str += f":{parsed.password}"
                    auth_str += "@"
                new_netloc = f"{auth_str}{ipv4}{port_str}"
                
                query_params = parse_qs(parsed.query)
                query_params['options'] = [f"endpoint={endpoint_id}"]
                new_query = urlencode(query_params, doseq=True)
                
                new_uri = urlunparse(parsed._replace(netloc=new_netloc, query=new_query))
                return new_uri
    except Exception:
        pass
    return uri

# ---------------------------------------------------------------------------
# Resolve DATABASE URI — always read from environment, never hardcoded.
# Set DATABASE_URI (or DATABASE_URL) in your .env file for each environment.
# ---------------------------------------------------------------------------
_raw_db_uri = os.environ.get("DATABASE_URI") or os.environ.get("DATABASE_URL")

if not _raw_db_uri:
    raise RuntimeError(
        "DATABASE_URI is not set. "
        "Copy backend/.env.example to backend/.env and fill in your NeonDB connection string."
    )

# Normalize postgres:// → postgresql:// (Heroku/Render style URLs)
if _raw_db_uri.startswith("postgres://"):
    _raw_db_uri = _raw_db_uri.replace("postgres://", "postgresql://", 1)


# ---------------------------------------------------------------------------
# Detect current environment
# ---------------------------------------------------------------------------
APP_ENV = os.environ.get("APP_ENV", "development").lower()  # development | qa | production
FLASK_ENV = os.environ.get("FLASK_ENV", "development").lower()


class Config:
    # ---- Security ----
    SECRET_KEY = os.environ.get("JWT_SECRET")
    if not SECRET_KEY:
        raise RuntimeError("JWT_SECRET is not set. Add it to your .env file.")

    # ---- Database ----
    SQLALCHEMY_DATABASE_URI = resolve_neon_uri(_raw_db_uri)
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    # Disable SQL echo in production/qa for cleaner logs
    SQLALCHEMY_ECHO = APP_ENV == "development"
    SQLALCHEMY_ENGINE_OPTIONS = {}

    # ---- Flask-Mail ----
    MAIL_SERVER = os.environ.get("MAIL_SERVER", "smtp.gmail.com")
    MAIL_PORT = int(os.environ.get("MAIL_PORT", 587))
    MAIL_USE_TLS = os.environ.get("MAIL_USE_TLS", "True").lower() in ("true", "1", "yes")
    MAIL_USE_SSL = os.environ.get("MAIL_USE_SSL", "False").lower() in ("true", "1", "yes")
    MAIL_USERNAME = os.environ.get("MAIL_USERNAME") or os.environ.get("EMAIL_ADDRESS")
    MAIL_PASSWORD = os.environ.get("MAIL_PASSWORD") or os.environ.get("EMAIL_APP_PASSWORD")

    _default_user = os.environ.get("MAIL_USERNAME") or os.environ.get("EMAIL_ADDRESS")
    MAIL_DEFAULT_SENDER = (
        os.environ.get("SMTP_FROM")
        or (f"SSJewellery <{_default_user}>" if _default_user else "SSJewellery <no-reply@SSJewellery.com>")
    )

    # ---- CORS ----
    # Comma-separated list of allowed frontend origins
    ALLOWED_ORIGIN = os.environ.get("ALLOWED_ORIGIN", "http://localhost:5173")
